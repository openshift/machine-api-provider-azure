...
var (
	// ErrResourceNotFound is returned when a item is not found in a list.
...
func newCache(azureClients actuators.AzureClients, location string) *Cache {
	return &Cache{
		client:   NewClient(azureClients),
		location: location,
	}
}
...
func GetCache(azureClients actuators.AzureClients, location string) (*Cache, error) {
	...
	c = newCache(azureClients, location)
