...
func getPlatformFaultDomain(vm *armcompute.VirtualMachine) *int32 {
	if vm.Properties == nil || vm.Properties.InstanceView == nil {
		return nil
	}
	return vm.Properties.InstanceView.PlatformFaultDomain
}
...
func castToVirtualMachine(vmInterface interface{}) (*armcompute.VirtualMachine, error) {
	...
	if !ok {
		return nil, fmt.Errorf("returned incorrect vm interface. expected armcompute.VirtualMachinesClientGetResponse, got %T", vmInterface)
	}
